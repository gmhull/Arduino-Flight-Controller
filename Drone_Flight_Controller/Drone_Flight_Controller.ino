...
float pid_p_gain_altitude = 1.2;           //Gain setting for the altitude p controller (1.2)
float pid_i_gain_altitude = 0.002;         //Gain setting for the altitude i controller (0.002)
float pid_d_gain_altitude = 1.5;           //Gain setting for the altitude d controller (1.5)
...
void setup(){
  ...
  DDRB |= B00001111;                                                        //Configure digital port 8, 9, 10 and 11 as output.
...
void loop(){
  ...
  if (flight_status >= 2){                                                  //The motors are started.
    ...
    if (flight_status == 3) {                                               //If altitude hold is active.
      throttle = 1000 + throttle_signal_center + pid_output_altitude;       //The base throttle is the receiver throttle channel + the detected take-off throttle + the PID controller output.
    }
  ...
  while((PORTB & B00001111) != 0){                                          //Stay in this loop until output 8,9,10 and 11 are low.
    ...
    if(timer_channel_1 <= esc_loop_timer) {
      PORTB &= B11111110;                //Set digital output 8 to low if the time is expired.
//      Serial.println(esc_loop_timer - loop_timer);
    }
